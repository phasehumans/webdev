...
class BigWave {
  ...
  constructor(w: number, h: number, index: number, total: number) {
    ...
    // Raised base position to allow waves to reach the prompt box area (approx 60% down the screen)
    this.baseY = h * 0.6
  ...
  update(w: number, h: number, time: number) {
    ...
    this.y =
      this.baseY + Math.sin(time * this.speedY + this.phase * 0.5) * (h * 0.1)
  }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    // Increased height scale to make them taller and reach higher
    ctx.scale(1.5, 1.4)

    const gradient = ctx.createRadialGradient(0, 0, 0, 0, 0, this.radius)

    // Gradient opacity adjustments for better visibility
...
export const DitherBackground = () => {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (parent) {
        // Ensure we use the window dimensions if parent is fixed/full screen
        canvas.width = Math.ceil(window.innerWidth / scale)
        canvas.height = Math.ceil(window.innerHeight / scale)

        // Reduced to 3 waves
        waves = Array.from({ length: 3 }).map(
          (_, i) => new BigWave(canvas.width, canvas.height, i, 3)
        )
      }
    }
    ...
    const loop = (time: number) => {
      ...
      for (let y = 0; y < h; y++) {
        const ny = y / h
        // Mask power adjustment to control vertical fade
        rowMasks[y] = Math.pow(Math.max(0, ny), 1.8)
      }
